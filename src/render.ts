import str from './str';
import { getCars, drive, startEngine, stopEngine, getWinners } from './api';
...
export const renderCar = ({ id, name, color }: { id: number; name: string; color: string }): string => `
 <div class="buttons">
 <button class="select" id="select-car-${id}">Select</button>
 <button class="remove" id="remove-car-${id}">Remove</button>
...
export const renderWinners = (): string => `
 ...
 <td>${
     winner.color === undefined
         ? (winner.color = `<svg xmlns="http://www.w3.org/2000/svg" version="1.0" width="1280.000000pt" height="640.000000pt" viewBox="0 0 1280.000000 640.000000" preserveAspectRatio="xMidYMid meet">
 ...
 <path d="M11035 1801 c-7 -12 -23 -144 -29 -243 -4 -77 -4 -78 19 -78 45 0 130 22 193 51 l64 29 -19 23 c-65 82 -198 227 -209 227 -7 0 -15 -4 -19 -9z"/>
         ...
         : renderColor(winner.color)
 }</td>
 <td>${winner.name === undefined ? (winner.name = 'Tesla') : winner.name}</td>
 <td>${winner.wins}</td>
 <td>${winner.time}</td>
 </tr>
 `
...
export const render = async () => {
	...
	<div class="garage-catalog">
		<form class="form" id="create">
			<input type="text" class="namecar" name="name" id="createname">
			<input type="color" class="color" name="color" value="#ffffff" id="createcolor">
			<button class="button" type="submit"> Create</button>
		</form>
		<form class="form" id="up">
			<input type="text" class="upnamecar" name="upname" id="upname" disabled>
			<input type="color" class="upcolor" name="color" value="#ffffff" id="upcolor" disabled>
			<button class="button-up" type="submit" id="upd" disabled> Update</button>
		</form>

		<div class="controls">
			<button class="race" id="race">Race</button>
